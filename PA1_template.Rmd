...
Calculate steps/day using aggregate() and then creating a histogram with hist(). 
...
The mean is `r meansteps` steps/day and the median is `r mediansteps` steps/day.
...
The interval with highest average amount of steps was interval `r maxinterval` with an average of `r maxinterval2`.

## Sixth question
Code to describe and show a strategy for imputing missing data.

Find how many NA's there is in the steps column of amd. Create another dataframe (amd2). Find the 5 min interval mean like before. 
Use a for loop to replace Na's with mean value. Count Na's again, should be 0. 
...
First find which days are which. Then subset based on if it's a weekday or the weekend. Find the mean in each 5-minute interval. 
